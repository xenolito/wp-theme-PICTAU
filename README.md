...
- La plantilla por defecto aplica `Cross-Origin-Resource-Policy: cross-origin` (en vez de `same-origin`) solo al directorio de subida de medios, calculado en runtime con `wp_upload_dir()` (nunca hardcodeado como `/xen_media/`, ya que cada instalación puede tener un `upload_path` distinto o el `wp-content/uploads` por defecto de WordPress), vía `SetEnvIf Request_URI` + `Header ... env=`. Necesario para que recursos de esa carpeta embebidos en un origen externo — como el logo generado para los [emails de CF7](#contact-form-7--plantilla-html-de-email-compatibilidad-outlook) — no queden bloqueados con el icono de "imagen no cargada" en el webview/motor de renderizado del cliente de correo del destinatario (Outlook Web, Spark...), ya que `same-origin` prohíbe la carga cross-origin aunque el fichero se sirva correctamente por HTTP.
