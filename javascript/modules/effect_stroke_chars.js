...
document.addEventListener('DOMContentLoaded', () => {
	...
	strokes.forEach(stroker => {
		...
		const inBetweenTargetSelector = stroker.dataset[hyphenToCamelcase(attInBetween.split('data-')[1])]

		const toStrokeElement = stroker.querySelector(toStrokeSelector)
		const inBetweenTargetElement = stroker.querySelector(inBetweenTargetSelector)
		...
		// copiamos las clases...
		strokedElement.setAttribute('class', `${toStrokeElement.classList.toString().replace(',', ' ')} stroked`)
