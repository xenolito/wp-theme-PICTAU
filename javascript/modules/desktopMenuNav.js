...
const makeMainNavSubmenusLayout = (headerContainerElement, visualPositioning) => {
	...
	const positionChildUnderParent = childElement => {
		...
		childElement.style.translate = `${parentPos.x + offsetX * 0.5 - submenuContainerPos.x}px 0`
		...
		if (offsetOverFlowRight) {
			...
			// }px`;
			childElement.style.translate = `${childElement.getBoundingClientRect().x - Math.round(offsetOverFlowRight) - submenuContainerPos.x}px 0`
		}
	}
