...
function addTypographyClasses() {
	const editorLoadedInterval = setInterval(function () {
		...
		if (Object.keys(targetClasses).every(className => document.getElementsByClassName(className).length)) {
			if (getCurrentPostTypeClass()) {
				// Add the post type class throughout.
				Object.values(targetClasses).forEach(className => className.push(getCurrentPostTypeClass()))
			}
