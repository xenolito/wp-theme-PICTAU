...
//? Check if the plugin 'Multilingual contact form 7 with Polylang' is active (https://es.wordpress.org/plugins/multilingual-contact-form-7-with-polylang/). If active, it will replace all '{' and '}' with ' ' in the email template, which will break our css styles.


$mcf7_polylang_active = false;
...
if (is_plugin_active('multilingual-contact-form-7-with-polylang/plugin.php')) {
	$mcf7_polylang_active = true;
}

function protegerLlavesSegunAnidamiento($input)
{
	$output = '';
	$depth = 0;
	$length = strlen($input);
	...
	for ($i = 0; $i < $length; $i++) {
		...
		if ($char === '{') {
			...
			if ($depth === 1) {
				// Marcamos inicio de bloque contenedor
				$stack[] = ['pos' => strlen($output), 'anidado' => false];
				$output .= '{{'; // bloque sin anidar por ahora
			} else {
				// Marcamos que el bloque actual es anidado
				if (!empty($stack)) {
					$stack[count($stack) - 1]['anidado'] = true;
				}
				$output .= '{';
			}
		} elseif ($char === '}') {
			if ($depth > 1) {
				$output .= '}}'; // cierre de bloque anidado
			} else {
				// Bloque contenedor sin anidamiento → cierre también duplicado
				$bloque = array_pop($stack);
				if (!$bloque['anidado']) {
					// Reescribimos apertura como {{ y cerramos como }}
					$output = substr_replace($output, '{{', $bloque['pos'], 2);
					$output .= '}}';
				} else {
					// Si fue bloque anidado → cerramos con llave simple
					$output .= '}';
				}
			}
			$depth--;
...
if ($mcf7_polylang_active) {
	// If the plugin is active, we need to protect the keys according to nesting
	// This will ensure that the CSS styles are not broken by the PLUGIN
	$htmlEmailTemplateCssStyles = protegerLlavesSegunAnidamiento($htmlEmailTemplateCssStyles);
}
