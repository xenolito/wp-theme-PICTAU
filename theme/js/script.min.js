(() => {
  ...
  var hasMarketingConsent = () => typeof window.gdpr_consent__advanced === "undefined" || window.gdpr_consent__advanced === "true";
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "modalform";
    const modals = document.querySelectorAll(`[data-${attributeId2}]`);
    ...
    modals.forEach((modal, index) => {
      const config3 = getConfigByAtt(modal, attributeId2, true);
...
Review your css class markup or html`);
  ...
  var NESTED_SCROLL_SELECTOR = "[data-overlayscrollbars-viewport], .fcal_slot_picker, .svelte-select-list, .fcal_date_event_details, .main-modal-content";
  var lenis = new Lenis({
  ...
  // javascript/modules/fluentbooking_timezone_dropdown_upward.js
  var WIDGET_SELECTOR = ".fluent_booking_app";
  var TRIGGER_SELECTOR = ".fcal_timezone_selector";
  var LIST_SELECTOR = ".svelte-select-list";
  var GAP = 6;
  ...
  function repositionList(list) {
    ...
    const top = openAbove ? Math.max(triggerRect.top - listHeight - GAP, VIEWPORT_MARGIN) : Math.min(triggerRect.bottom + GAP, window.innerHeight - listHeight - VIEWPORT_MARGIN);
    const topPx = `${top}px`;
    if (list.style.top !== topPx) list.style.top = topPx;
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "anim_any";
    const headerToAnim = document.querySelectorAll(`[data-${attributeId2}]`);
      ...
      setRotateXAnimation = () => {
        ...
        this.timeLine.fromTo(
          ...
          { opacity: 0, rotationX: this.rotateXStartAngle },
          {
            ...
            ease: "power3.out",
            clearProps: "transform"
            // ver comentario en setSlideTo: evita atrapar hijos position:fixed
          }
        );
      };
      setZoomInAnimation = () => {
    ...
    const mergeGroups = (a2, b2) => {
      const groupA = getGroup(a2);
      const groupB = getGroup(b2);
    ...
    headerToAnim.forEach((header) => {
      const config3 = getConfigByAtt(header, attributeId2);
      if (config3.chainanim) {
        config3.chainanim.split(",").forEach((sel) => {
          const target = header.parentElement?.parentElement?.querySelector(sel.trim());
          if (target && target.hasAttribute(`data-${attributeId2}`)) mergeGroups(header, target);
        });
      }
      if (config3.nextanim) {
        const selector3 = config3.nextanim.split(",")[0].trim();
        const target = document.querySelector(selector3);
        if (target && target.hasAttribute(`data-${attributeId2}`)) {
    ...
    const instantiateGroup = (group) => {
      Array.from(headerToAnim).filter((header) => group.includes(header)).forEach((header) => {
        const config3 = getConfigByAtt(header, attributeId2);
    ...
    const needsEagerInstantiation = (header) => {
      const config3 = getConfigByAtt(header, attributeId2);
      return config3.autoplay === "0" || config3.autoplay === "false" || chainedTargets.has(header);
    };
    const uniqueGroups = new Set(groupOf.values());
    const deferredGroups = /* @__PURE__ */ new Set();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "marquee";
    const marqueeContainer = document.querySelectorAll(`[data-${attributeId2}]`);
...
${this.items.length} found!`);
    ...
    marqueeContainer.forEach((marqueeContainer2) => {
      const config3 = getConfigByAtt(marqueeContainer2, attributeId2, false);
  ...
  // javascript/modules/testimonials-splide.js
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "testimonials";
    const testimonialsContainer = document.querySelectorAll(`[data-${attributeId2}]`);
    ...
    const Testimonials = class {
      constructor(testimonialsContainer2, config3 = {}) {
        ...
        this.lazyload = "testimonials_lazyload" in testimonialsContainer2.dataset ? "nearby" : false;
...
\u{1F4E6} Group (outer)    \u2190 data-testimonials aqu\xED
  ...
  \u2514\u2500\u2500 \u{1F4E6} Group (slide N\u2026)`
      ...
      initSplide = () => {
        ...
        if (this.lazyload) {
          config3.lazyLoad = this.lazyload;
          config3.preloadPages = this.preloadPages;
        }
        try {
    ...
    testimonialsContainer.forEach((testimContainer) => {
      ...
      const config3 = getConfigByAtt(testimContainer, attributeId2, true);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "heroslider";
    const containers = document.querySelectorAll(`[data-${attributeId2}]`);
    ...
    containers.forEach((container) => {
      const config3 = getConfigByAtt(container, attributeId2, true);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const safeQuerySelector = (selector3) => {
      try {
    ...
    aNames.forEach((a2) => {
      ...
      if (linkToSamePage(a2) && (hashPart === "top" || safeQuerySelector(`#${hashPart}`))) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "socialshare";
    const socialButton = document.querySelectorAll(`[data-${attributeId2}]`);
    ...
    socialButton.forEach((socialButton2) => {
      const config3 = getConfigByAtt(socialButton2, attributeId2, true);
      socialButton2.SocialShareButton = new SocialShareButton(socialButton2, config3);
    });
  });

  // javascript/modules/webgl_dots_connected_anim.js
  var import_throttleit = __toESM(require_throttleit());
  gsap.registerPlugin(ScrollTrigger2);
  var DEFAULT_COLOR = { r: 156, g: 217, b: 249 };
  var parseColor = (value, fallback = DEFAULT_COLOR) => {
    ...
    const rgb = value.split(",").map((n2) => Number(n2.trim()));
    if (rgb.length === 3 && rgb.every((n2) => !Number.isNaN(n2))) {
      return { r: rgb[0], g: rgb[1], b: rgb[2] };
    }
    return fallback;
  };
  document.addEventListener("DOMContentLoaded", () => {
    const attributeId2 = "webgldots";
    const backgroundContainer = document.querySelectorAll(`[data-${attributeId2}]`);
    ...
    const BackgroundGL = class {
      ...
      createPoints = () => {
        const numberOfPoints = Math.floor(this.targetRect.width * this.targetRect.height / 1e4) * this.density;
        this.points = [];
        for (let x2 = 0; x2 < numberOfPoints; x2++) {
          const px = Math.random() * this.size.w - this.size.w * 0.5;
          const py = Math.random() * this.size.h - this.size.h * 0.5;
          const p2 = { x: px, originX: px, y: py, originY: py };
          this.points.push(p2);
        }
        for (let i3 = 0; i3 < this.points.length; i3++) {
          const closest3 = [];
          const p1 = this.points[i3];
          for (let j2 = 0; j2 < this.points.length; j2++) {
            const p2 = this.points[j2];
            if (!(p1 == p2)) {
              let placed = false;
              for (let k2 = 0; k2 < 5; k2++) {
                if (!placed) {
                  if (closest3[k2] == void 0) {
                    closest3[k2] = p2;
                    placed = true;
                  }
                }
              ...
              for (let k2 = 0; k2 < 5; k2++) {
                if (!placed) {
                  if (this.getDistance(p1, p2) < this.getDistance(p1, closest3[k2])) {
                    closest3[k2] = p2;
                    placed = true;
                  }
                }
        ...
        for (let i3 in this.points) {
          const c2 = new Circle(this, this.points[i3], 1.5 + Math.random() * this.dotSize, "rgba(255,255,255,0.3)");
          this.points[i3].circle = c2;
        }
      };
      animate = () => {
        if (!this.paused) {
          ...
          for (let i3 in this.points) {
            const translatedPos = {
              x: this.points[i3].x + this.targetOrigin.x - this.size.w * 0.5,
              // Center the points in the canvas
              y: this.points[i3].y + this.targetOrigin.y - this.size.h * 0.5
            };
            const opacity = this.getAspectRatioOpacity(this.points[i3].x, this.points[i3].y, 0, 0, this.size.w * this.maskspread, this.size.h * this.maskspread);
            this.points[i3].active = opacity * 0.15;
            this.points[i3].circle.active = opacity;
            this.points[i3].circle.draw();
            this.drawLines(this.points[i3]);
          }
      ...
      drawLines = (p2) => {
        if (!p2.active) return;
        for (let i3 in p2.closest) {
          if (!p2.closest[i3] || p2.closest[i3].active < 0.08) continue;
          this.ctx.beginPath();
          this.ctx.moveTo(p2.x + this.targetOrigin.x, p2.y + this.targetOrigin.y);
          this.ctx.lineTo(p2.closest[i3].x + this.targetOrigin.x, p2.closest[i3].y + this.targetOrigin.y);
          this.ctx.strokeStyle = `rgba(${this.colorlines.r},${this.colorlines.g},${this.colorlines.b},${p2.active})`;
          this.ctx.stroke();
        }
      };
      shiftAllPoints = () => {
        for (let i3 in this.points) {
          this.shiftPoint(this.points[i3]);
        }
      };
      shiftPoint = (p2) => {
        gsap.to(p2, {
          ...
          x: p2.originX + this.randomBetweenNegXAndX(this.maxdisplacement),
          y: p2.originY + this.randomBetweenNegXAndX(this.maxdisplacement),
      ...
      randomBetweenNegXAndX = (x2) => {
        return Math.random() * (2 * x2) - x2;
      };
      getAspectRatioOpacity = (x2, y2, originX, originY, width, height) => {
        const dx = (x2 - originX) / (width / 2);
        const dy = (y2 - originY) / (height / 2);
    ...
    const Circle = class {
      constructor(ref2, pos, rad, color) {
        this.ref = ref2;
        this.pos = pos || null;
    ...
    backgroundContainer.forEach((bgContainer) => {
      const config3 = getConfigByAtt(bgContainer, attributeId2, false);
