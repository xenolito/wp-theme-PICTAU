  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const HeaderAnimation = class {
      ...
      applyFixedWords = (items, splitOf) => {
        ...
        const n2 = Math.min(this.fixedWords, words.length);
        if (n2 < this.fixedWords) {
          console.warn(`anim_any cyclecontentinline: data-anim_any_fixedwords pide ${this.fixedWords} palabra(s) pero el primer hijo solo tiene ${words.length}, se usan ${n2}`);
        }
        const boundaryNode = words[n2] || null;
        const container = (boundaryNode ?? words[n2 - 1]).parentElement;
        ...
        words.splice(0, n2);
        splitOf.get(firstItem).chars = splitOf.get(firstItem).chars.filter((c2) => firstItem.contains(c2));
      };
